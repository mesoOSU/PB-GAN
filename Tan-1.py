...
This is code for regularization on the generation by adding the Tan-1() method to the GAN loss
"""
...
def define_discrim(image_shape = (128,128,3)):
    ...
    optd = Adam(learning_rate=0.0001, beta_1=0.5)
    d_model.compile(loss='binary_crossentropy', optimizer=optd, loss_weights=[0.8])
    return d_model
...
def tan1_func(y_true, y_pred):
    div_pred = calc_divergence(y_pred)
    div_true = calc_divergence(y_true)
    
    lossi =  tf.math.sqrt(tf.reduce_mean(tf.math.square(div_pred[0]))) 
    lossii = tf.math.sqrt(tf.reduce_mean(tf.math.square(div_pred[1])))
    Tlossi =  tf.math.sqrt(tf.reduce_mean(tf.math.square(div_true[0]))) 
    Tlossii = tf.math.sqrt(tf.reduce_mean(tf.math.square(div_true[1])))
    # tf.math.atan(lossi)-Tlossi and tf.math.atan(lossi-Tlossi) are essentially (though not technically ) the same
    # for our small values. tf.math.atan(lossi)-Tlossi performs slightly better than tf.math.atan(lossi-Tlossi)
    # maybe becuase the penalty is less harsh
    loss1 = 2*tf.math.abs(tf.math.atan(lossi)-Tlossi) + 2*tf.math.abs(tf.math.atan(lossii)-Tlossii)
    
    loss0 = tf.keras.losses.mean_squared_error(y_true, y_pred)
    
    loss = 300*loss0 + loss1
    return loss
...
def define_gan(g_model, image_shape):
    # make weights in the discriminator not trainable expect when batch norm is applied (so you can get the batch statistics)
	...
	# compile model
    optg = Adam(learning_rate=0.0003, beta_1=0.5) 
    model.compile(loss=['binary_crossentropy', tan1_func], optimizer=optg, loss_weights=[1,1])
    return model
