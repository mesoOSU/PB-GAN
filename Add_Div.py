...
This is code for regularization on the generator by adding the divergence to the GAN loss
"""
...
save_npz_name = 'Add_div-wt=50_D_G_LR=0.0002_Dwt=0.5_MAE'
...
def define_discrim(image_shape = (128,128,3)):
    ...
    optd = Adam(learning_rate=0.0002, beta_1=0.5)
...
def adding_divergence(y_true, y_pred):
    div_pred = calc_divergence(y_pred)
    L = 100                                                                     # weight of L1 loss term
    LL = 50                                                                     # weight of adding divergence regularization term
    # loss0 = tf.keras.losses.mean_squared_error(y_true, y_pred)                # L1 loss term
    loss0 = tf.keras.losses.mean_absolute_error(y_true, y_pred)

    lossi = tf.math.abs(div_pred[0]) 
    lossii = tf.math.abs(div_pred[1])
    loss1 = lossi + lossii
...
def define_gan(g_model, image_shape):
    ...
    model.compile(loss=['binary_crossentropy', adding_divergence], optimizer=optg, loss_weights=[1,1])
    return model
