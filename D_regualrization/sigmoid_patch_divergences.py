...
this code uses regyualrization on the discriminator throught the sigmoid function on a patch basis. The divergence fields are calculated, then the 
RMS is calculated on the divergence fields on a patch basis. A probability is assigned to each RMS patch (of that RMS score being real) and then the 
overall probability is multiplied by the output of the discriminator to get the final probability 
...
def calc_divergence(input):
    ...
    x1 = tf.linspace(tf.cast(0, dtype=tf.float32), (Nx/2)-1, num=int(Nx/2))
    ...
    y1 = tf.linspace(tf.cast(0, dtype=tf.float32), (Nx/2)-1, num=int(Nx/2))
...
def get_patches(stress_fields):
    patches = []
    numbers = [0,16,32,48,64,80,96,112,128]
    for p in numbers:
        for k in numbers:
            for r in numbers:
                for j in numbers:
                    patch1 = stress_fields[:,int(p):int(k), int(r):int(j)]
    ...
    for h in patches:
        ...
        if hh==16:
            if hhh==16:
                patches_final.append(h)
    return patches_final


def sig_expo_fn(div1_patches, div2_patches):
    #div1, div2 = calc_divergence(stress_fields)
    rms0 = tf.math.sqrt(tf.reduce_mean(tf.math.square(div1_patches)))
    rms1 = tf.math.sqrt(tf.reduce_mean(tf.math.square(div2_patches)))
    h = 10
...
def get_div_patch(stress_fields):
    div1, div2 = calc_divergence(stress_fields)
    div1_patches = get_patches(div1)
    div2_patches = get_patches(div2)
    sig_out = []
    for i in range(0, len(div1_patches)):
        rmss = sig_expo_fn(div1_patches[i], div2_patches[i])
        sig_out.append(rmss)
    # print('sig out shape:', tf.convert_to_tensor(sig_out).shape)
    sig_out = tf.reshape(tf.convert_to_tensor(sig_out), [-1,8,8,1])
    # print('div_patch shape', sig_out.shape)
    return sig_out
...
print('patch out shape', patch_out_hidden.shape)
div_prob = get_div_patch(in_target_image)
patch_out_new = Lambda(lambda x: x*div_prob)(patch_out_hidden)
...
def define_generator(image_shape=(128,128,3)):
    ...
    out_image = Reshape(image_shape)(out_image)
...
def define_gan(g_model, image_shape):
    ...
    div_prob = get_div_patch(gen_out)
    dis_out = Lambda(lambda x: x*div_prob)(gan_out)
	# src image as input (in_src), generated image (gen_out) and classification output (dis_out)
